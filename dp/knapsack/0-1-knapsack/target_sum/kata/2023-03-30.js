...
const targetSum = (nums, target) => {
    ...
    const backtrack = (currentSum = 0, currentIndex = 0) => {
        ...
        return backtrack(currentSum - nums[currentIndex], currentIndex + 1) + backtrack(currentSum + nums[currentIndex], currentIndex + 1)
...
function findTargetSumWays(nums, target) {
    ...
    let offset = sum;
    let dp = new Array(nums.length + 1).fill(null).map(() => new Array(2 * sum + 1).fill(0));
    dp[0][offset] = 1;
